...
$(document).ready(function () {
  ...
  (async () => {
    ...
    try {
      const response = await ajaxRequest('GET', '/requests/line_chart_by_month');
      if (response && response.data) {
        ...
        for (const key in response.data) {
          if (Object.hasOwnProperty.call(response.data, key)) {
            ...
            line_labels.push(months[key - 1])
            data_vals.push(element)
          }
        }
              ...
              tooltips: {
                callbacks: {
                  ...
                  label: function (tooltipItem, data) {
                    return  data["datasets"][0]["data"][tooltipItem["index"]] + ' requests';
                  }
                },
                responsive: true,
                intersect: false,
...
(async () => {
  ...
  try {
    const response = await ajaxRequest('GET', '/users/bar_graph_by_month');
    ...
    if (response && response.data) {
      for (const key in response.data) {
        if (Object.hasOwnProperty.call(response.data, key)) {
          const element = response.data[key];
          bar_labels.push(`${key}`)
          active_labels.push(element.active)
          inactive_labels.push(element.inactive)
        }
      }
